...
ANSWER_KEY = {0: 0, 1: 3, 2: 2, 3: 1, 4: 4, 5: 3, 6: 3, 7: 0}

# load image, convert to grayscale, blur slightly, find edges
...
# apply a four point perspective transform to both the original image and grayscale image to
...
# find contours in the thresholded image, then initialize the list of contours that correspond
# to questions
...
for cnt in cnts:
	# compute the bounding box of the contourm then use the bounding box to derive the aspect
	# ratio
...
# sort the question contours top-to-bottom, then initialize the total number of correct
# answers
questionCnts = contours.sort_contours(questionCnts, method="top-to-bottom")[0]
correct = 0

# each question has 5 possible answers, to loop over the question in batches of 5
for (question, i) in enumerate(np.arange(0, len(questionCnts), 5)):
	# sort the contours of the current question from left to right, then intialize the index of
	# the bubbled answer
	...
	for (j, c) in enumerate(cnts):
		...
		# apply the mask to the thresholded image, then count the number of non-zero pixels in
		# the bubble area
